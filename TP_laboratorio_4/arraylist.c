...
ArrayList* al_clone(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        ...
        returnAux=al_subList(pList,0,pList->size);
    }
...
int resizeUp(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        ...
        {
            Aux_pElements=realloc(pList->pElements,(pList->reservedSize + AL_INCREMENT)*sizeof(void*));
            if(Aux_pElements!=NULL)
            {
                pList->pElements=Aux_pElements;
...
int expand(ArrayList* pList,int index)
{
    int returnAux = -1;
    int i;
    if (pList!=NULL && index>=0 && index<=pList->size && pList->size>0)
    {
...
int resizeDown(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
        ...
        {
            Aux_pElements=realloc(pList->pElements,(pList->size+AL_INITIAL_VALUE)*sizeof(void*));
            if(Aux_pElements!=NULL)
            {
                pList->pElements=Aux_pElements;
                pList->reservedSize=pList->size + AL_INITIAL_VALUE;
            }
        }
        returnAux=0;
    }
...
ArrayList* SublistA(ArrayList* pList, int(*pFunc)(void*))
{
    ArrayList* returnAux = NULL;
    ...
    returnAux=pList->subList(pList,value1,value2);

    if(pList!=NULL && pFunc!=NULL && returnAux!=NULL)
    {
        for(i=0; i<size; i++)
        {
            if(pFunc(pList->get(pList,i))==1)
            {
                pList->add(returnAux,pList->get(pList,i));
            }
        }
